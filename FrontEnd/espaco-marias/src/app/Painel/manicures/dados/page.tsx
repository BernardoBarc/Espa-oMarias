...
export default function EditarManicures() {
  ...
  // Estados e flags para verificação (adicionados)
  ...
  // Funções de verificação (similar ao fluxo do cliente)
  const startEmailVerification = async (tempId?: string) => {
    ...
    try {
      ...
      setInfo(`Código de verificação enviado para ${form.email}.`);
    } catch (err) {
      console.error(err);
  ...
  const confirmEmailCode = async () => {
    ...
    try {
      ...
      const emailToVerify = verifiedEmail || form.email; // Usar o email que foi enviado para verificação
      ...
      try {
        ...
        if (updateResponse.ok) {
          setEmailVerificationPending(false);
          setEmailVerifiedForEdit(false); // Reset porque já foi salvo
          setVerifiedEmail("");
          setEmailCode("");
  ...
  const confirmPhoneCode = async () => {
    ...
    try {
      ...
      const phoneToVerify = verifiedPhone || form.phone; // Usar o telefone que foi enviado para verificação
      ...
      try {
        ...
        if (updateResponse.ok) {
          setPhoneVerificationPending(false);
          setPhoneVerifiedForEdit(false); // Reset porque já foi salvo
          setVerifiedPhone("");
          setPhoneCode("");
      ...
      <div className="flex-1 px-6 py-8 relative z-10">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-center text-gray-300 mb-8">
            Gerencie suas informações pessoais e profissionais de forma segura
          </p>
